...
if ~exist('EnergyDiss2D','dir'); mkdir('EnergyDiss2D'); end
...
energy = zeros(Nt+1,1);
E2D = @(phi_sol,Nx,Ny,h,eps) ...
    0.5*(sum((phi_sol(2:Nx+2,2:Ny+1)-phi_sol(1:Nx+1,2:Ny+1)).^2,'all') ...
    +sum((phi_sol(2:Nx+1,2:Ny+2)-phi_sol(2:Nx+1,1:Ny+1)).^2,'all')) ...
    +(h^2/(4*eps^2))*sum((1-phi_sol(2:Nx+1,2:Ny+1).^2).^2,'all');
energy(1) = E2D(phi,Nx,Ny,h,eps);

for it = 1:Nt
    phi(1,:) = phi(2,:); phi(Nx+2,:) = phi(Nx+1,:);
    phi(:,1) = phi(:,2); phi(:,Ny+2) = phi(:,Ny+1);

    f = phi(2:Nx+1,2:Ny+1)-phi(2:Nx+1,2:Ny+1).^3;
    C = sum(f,'all')/sum((1-phi(2:Nx+1,2:Ny+1).^2),'all');
    lap = (phi(1:Nx,2:Ny+1)+phi(3:Nx+2,2:Ny+1) ...
        +phi(2:Nx+1,1:Ny)+phi(2:Nx+1,3:Ny+2)-4*phi(2:Nx+1,2:Ny+1))/h^2;
    phi(2:Nx+1,2:Ny+1) = phi(2:Nx+1,2:Ny+1)+dt*(f/eps^2+lap ...
        -C*(1-phi(2:Nx+1,2:Ny+1).^2)/eps^2);

    energy(it+1) = E2D(phi,Nx,Ny,h,eps);
end
t = [0:Nt]*dt;
...
yticks([0 0.5 1])
drawnow
print('EnergyDiss2D/normalized_energy_2D_log.eps','-depsc')
